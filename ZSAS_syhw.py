...
folder_path = './dataset/{}/{}/test/'.format(dataset_name, main_name)
gt_folder_path = './dataset/{}/{}/ground_truth/'.format(dataset_name, main_name)
sub_names = [x for x in os.listdir(folder_path) if x != 'good']

saa_sub_values = []
naive_sub_values = []
ram_llama_sub_values = []
syhw_sub_values = []

for sub_name in sub_names:
    print('--------------------{} test is starting...-------------------------'.format(sub_name))
    path = os.path.join(folder_path, sub_name)
    gt_path1 = os.path.join(gt_folder_path, sub_name)
    sub_numbers= os.listdir(path)
    ...
    for sub_number in sub_numbers:
        ...
        image_path = os.path.join(path, sub_number)
        gt_path2 = os.path.join(gt_path1, '{}_mask.png'.format(sub_number.split('.')[0]))
        print(image_path)
        print(gt_path2)
        
        source_image, raw_image, ram_image, image, gt_image = load_image(image_path, gt_path2)
        gt_image = cv2.imread(gt_path2, cv2.IMREAD_GRAYSCALE)
        ...
        size_threshold = [size_tuple[0] - 0.005, size_tuple[1] - 0.005]
    ...
    saa_sub_values.append((sub_name, [round(sum(column) / len(column), 4) for column in zip(*saa_values)]))
    naive_sub_values.append((sub_name, [round(sum(column) / len(column), 4) for column in zip(*naive_values)]))
    ram_llama_sub_values.append((sub_name, [round(sum(column) / len(column), 4) for column in zip(*ram_llama_values)]))
    syhw_sub_values.append((sub_name, [round(sum(column) / len(column), 4) for column in zip(*syhw_values)]))


saa_sub_values_file = './results_csv_sy/{}_{}_saa.csv'.format(dataset_name, main_name)
naive_sub_values_file = './results_csv_sy/{}_{}_naive.csv'.format(dataset_name, main_name)
ram_llama_sub_values_file = './results_csv_sy/{}_{}_ram_llama.csv'.format(dataset_name, main_name)
syhw_sub_values_file = './results_csv_sy/{}_{}_syhw.csv'.format(dataset_name, main_name)

def save_to_csv(file_path, data, headers):
    with open(file_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(headers)  # 헤더 추가
        writer.writerows(data)

headers = ['subname', 'iou', 'accuracy', 'precision', 'recall', 'f1_score']

# 데이터 저장
save_to_csv(saa_sub_values_file, saa_sub_values, headers)
save_to_csv(naive_sub_values_file, naive_sub_values, headers)
save_to_csv(ram_llama_sub_values_file, ram_llama_sub_values, headers)
save_to_csv(syhw_sub_values_file, syhw_sub_values, headers)

print(saa_sub_values)
print(naive_sub_values)
print(ram_llama_sub_values)
print(syhw_sub_values)